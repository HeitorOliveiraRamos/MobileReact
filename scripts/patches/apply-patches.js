/*
  Applies compatibility fixes for third-party packages.
  Currently patches react-native-document-picker (9.3.1) for RN 0.81 Android:
  - Replaces removed GuardedResultAsyncTask with a background Thread + Runnable
*/

const fs = require('fs');
const path = require('path');

function patchRNDocumentPicker() {
  ...
  if (original.includes('private static class ProcessDataTask implements Runnable')) {
    return;
  }

  const patched = `package com.reactnativedocumentpicker;
...
`;
  ...
  fs.writeFileSync(javaPath, patched, 'utf8');
}

function main() {
  try {
    patchRNDocumentPicker();
